                    ...
                    <Table className="table table-container table-bordered" striped bordered hover>
                        ...
                        <tbody className="bigger-card">
                            {transactions.map(transaction => (
                                <tr key={transaction.id} onClick={() => router.push(`/manage/transactions/${transaction.id}`)}
                                    className={Date.parse(transaction.pay_date) < Date.now() && !transaction.appointment_date ? 'table-danger td-style' : 'td-style'}
